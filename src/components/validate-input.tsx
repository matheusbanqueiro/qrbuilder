    ...
    return (
      <div className="relative">
        <input
          ...
          type={inputType}
          className={cn(
            `flex h-10 w-full rounded-md bg-white ring-1 ring-ring ring-offset-0 focus:outline-none focus:ring-2 ${
              errorMessage ? "ring-red-500/80" : "focus:ring-brand-secondary/80"
            }  px-3 py-2 text-sm text-neutral-900 file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-neutral-400  disabled:cursor-not-allowed disabled:opacity-50`,
            className
          )}
          ref={ref}
          {...props}
        />
        {disabled && (
          <LockClosedIcon className="w-5 h-5 absolute right-3.5 top-3.5 text-neutral-400/50" />
        )}
        {errorMessage && (
